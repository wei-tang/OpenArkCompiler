...
from task import RunCaseTask, RunCasesTask, GenShellScriptTask
...
# TODO: Add executor
target_args.add_option('-j', '--jobs', dest="jobs", type=int, default=cpu_count(), help='Number of parallel jobs')
...
def main(orig_args):
    ...
    if clean:
        if len(case_names) == 1:
            Case(list(case_names)[0], 'clean').clean(detail=True)
        else:
            for case_name in case_names:
                Case(case_name, 'clean').clean(detail=False)
        sys.exit(0)

    if save:
        if len(case_names) == 1:
            Case(list(case_names)[0], 'save').save_tmp()
        else:
            for case_name in case_names:
                Case(case_name, 'save').save_tmp()
        sys.exit(0)
