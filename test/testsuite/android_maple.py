...
def do_prepare(components, info, maple_root):
    ...
    maple_android_path = maple_root + "/android"
    gnu_bin_path = maple_root + "/tools/android-ndk-r21/toolchains/llvm/prebuilt/linux-x86_64/bin/"

    # 编译选项 java2d8和dex2mpl没有
    # javac_options = "-g -d . "
    # jar_options = "-cvf "

    as_options = "-g3 -O2 -x assembler-with-cpp -march=armv8-a -target aarch64-linux-android -c "
    cc_options = "-g3 -O2 -c -march=armv8-a -target aarch64-linux-android"
    cxx_options = "-g3 -O2 -c -fPIC -march=armv8-a -target aarch64-linux-android"
    linker_options_head = "-march=armv8-a -target aarch64-linux-android -shared -o "\
                        + output_name + " " + maple_android_path + "/out/soong/.intermediates/bionic/libc/crtbegin_so/android_arm64_armv8-a/crtbegin_so.o "\
                        + maple_out_lib_path + "/mrt_module_init.o "\
                        + maple_root + "/tools/clang-r353983c/lib64/clang/9.0.3/lib/linux/libclang_rt.builtins-aarch64-android.a "\
                        + maple_root + "/tools/gcc/linux-x86/aarch64/aarch64-linux-android-4.9/aarch64-linux-android/lib64/libatomic.a "\
                        + maple_root + "/tools/gcc/linux-x86/aarch64/aarch64-linux-android-4.9/lib/gcc/aarch64-linux-android/4.9.x/libgcc.a "\
                        + maple_android_path + "/out/soong/.intermediates/bionic/libc/crtend_so/android_arm64_armv8-a/obj/bionic/libc/arch-common/bionic/crtend_so.o "\
                        + "-fuse-ld=lld -lcore-all -lc -ldl -lcommon_bridge -lc++ -lm -Wl,-T "\
                        + maple_out_lib_path + "/linker/maplelld.so.lds "\
                        + maple_root + "/tools/clang-r353983c/lib64/clang/9.0.3/lib/linux/libclang_rt.ubsan_minimal-aarch64-android.a "\
                        + "-Wl,--exclude-libs,libclang_rt.ubsan_minimal-aarch64-android.a -nostdlib"
    ...
    components["ld"].update_info(None, linker_options_head)
