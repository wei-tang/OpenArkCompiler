...
uint32 PreWorkCandHashTable::ComputeWorkCandHashIndex(const MeExpr &meExpr) {
  ...
  switch (meOp) {
    ...
    case kMeOpVar: {
      ...
      hashIdx = static_cast<uint32_t>(varMeExpr.GetOst()->GetIndex()) << kOffsetVarMeExprOstIdx;
      break;
    }
    case kMeOpReg: {
...
uint32 PreWorkCandHashTable::ComputeStmtWorkCandHashIndex(const MeStmt &stmt) {
  ...
  switch (stmt.GetOp()) {
    ...
    case OP_dassign: {
      ...
      hIdx += static_cast<uint32_t>(varMeExpr->GetOst()->GetIndex()) << kOffsetVarMeExprOstIdx;
    ...
    case OP_callassigned: {
      ...
      if (!callAss.GetMustDefList().empty()) {
        ...
        hIdx += static_cast<uint32>(lhsVar->GetOst()->GetIndex()) << 1;
      }
      break;
    }
    default:
