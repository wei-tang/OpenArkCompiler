 ...
 */
bool LSRALinearScanRegAllocator::PropagateRenameReg(Insn &nInsn, uint32 replaceReg, Operand &renameOpnd) {
  uint32 renameReg = static_cast<RegOperand*>(&renameOpnd)->GetRegisterNumber();
  ...
  for (int32 i = lastOpndId; i >= 0; --i) {
    ...
    if (opnd.IsList()) {
      ...
      bool isDef = static_cast<AArch64OpndProp*>(md->operand[i])->IsRegDef();
      auto &regOpnd = static_cast<RegOperand&>(opnd);
      uint32 regCandidate = regOpnd.GetRegisterNumber();
      if (isDef) {
        /* Continue if both replace_reg & rename_reg are not redefined. */
        if (regCandidate == replaceReg || regCandidate == renameReg) {
          return true;
        }
      } else {
        if (regCandidate == replaceReg) {
          nInsn.SetOperand(i, renameOpnd);
        }
      }
    }
  }
 ...
 */
void LSRALinearScanRegAllocator::PropagateX0() {
  FOR_ALL_BB(bb, cgFunc) {
     ...
     */
    if (insn == nullptr || (insn->GetMachineOpcode() != MOP_xmovrr && insn->GetMachineOpcode() != MOP_wmovrr)) {
      continue;
    }
    auto &movSrc = static_cast<RegOperand&>(insn->GetOperand(1));
    if (movSrc.GetRegisterNumber() != R0) {
      continue;
    }

    /* At this point the 1st insn is a mov from x0. */
    auto &movDst = static_cast<RegOperand&>(insn->GetOperand(0));
    uint32 replaceReg = movDst.GetRegisterNumber();

    bool redefined = false;
    Insn *renameInsn = nullptr;
    Operand *renameOpnd = nullptr;
    uint32 renameReg = 0;
    for (Insn *nInsn = insn->GetNext(); nInsn != nullptr; nInsn = nInsn->GetNext()) {
       ...
       */
      uint32 numResults = nInsn->GetResultNum();
      const AArch64MD *md = &AArch64CG::kMd[static_cast<AArch64Insn*>(nInsn)->GetMachineOpcode()];
      if (numResults == 1) {
        bool isDef = static_cast<AArch64OpndProp*>(md->operand[0])->IsRegDef();
        if (isDef) {
          auto &opnd = static_cast<RegOperand&>(nInsn->GetOperand(0));
          if (opnd.GetRegisterNumber() == replaceReg) {
            redefined = true;
          }
        }
      } else if (numResults > 1) {
        ASSERT(numResults <= nInsn->GetOperandSize(), "numResults shouldn't be more than the operands' size here");
        for (uint32 i = 0; i < numResults; ++i) {
          bool isDef = static_cast<AArch64OpndProp*>(md->operand[i])->IsRegDef();
          if (isDef) {
            auto &opnd = static_cast<RegOperand&>(nInsn->GetOperand(i));
            if (opnd.GetRegisterNumber() == replaceReg) {
              redefined = true;
              break;
            }
      ...
      if (srcReg.GetRegisterNumber() != replaceReg) {
        if (redefined) {
          break;
        }
        continue;
      ...
      auto &dstReg = static_cast<RegOperand&>(dst);
      if (dstReg.GetRegisterNumber() != R0) {
        /* This is to set up for further propagation later. */
        if (srcReg.GetRegisterNumber() == replaceReg) {
          if (renameInsn != nullptr) {
            redefined = true;
            break;
          } else {
            renameInsn = nInsn;
            renameOpnd = &dst;
            renameReg = dstReg.GetRegisterNumber();
          }
        }
        continue;
      }
      ...
      nInsn->SetOperand(1, movSrc);
      break;
    }

    if (redefined || renameInsn == nullptr) {
      continue;
    }
     ...
     */
    if (renameReg < kAllRegNum) {  /* dont propagate physical reg */
      continue;
    }
    ...
    if (bb->GetLiveOutRegNO().find(replaceReg) == bb->GetLiveOutRegNO().end() ||
        bb->GetLiveOutRegNO().find(renameReg) == bb->GetLiveOutRegNO().end() ||
        nextBB->GetLiveOutRegNO().find(replaceReg) != nextBB->GetLiveOutRegNO().end()) {
      continue;
    }

    /* Replace replace_reg by rename_reg. */
    for (Insn *nInsn = renameInsn->GetNext(); nInsn != nullptr && nInsn != bb->GetLastInsn();
         nInsn = nInsn->GetNext()) {
      if (!nInsn->IsMachineInstruction()) {
        continue;
      }
      redefined = PropagateRenameReg(*nInsn, replaceReg, *renameOpnd);
      if (redefined) {
        break;
      }
    }
    ...
    for (Insn *nInsn = nextBB->GetFirstInsn(); nInsn != nextBB->GetLastInsn(); nInsn = nInsn->GetNext()) {
      if (!nInsn->IsMachineInstruction()) {
        continue;
      }
      redefined = PropagateRenameReg(*nInsn, replaceReg, *renameOpnd);
...
bool LSRALinearScanRegAllocator::AllocateRegisters() {
  ...
  if (CGOptions::DoPreLSRAOpt()) {
    PropagateX0();
    if (LSRA_DUMP) {
      LogInfo::MapleLogger() << "******** CG IR After PreLSRA: *********" << '\n';
      cgFunc->DumpCGIR();
    }
  }
